...
\item	0\% \textless  Livello batteria \textless= 50\%  \ding{226} 1 pull ogni 20 secondi
\end{itemize}
...
\begin{itemize}
\item{Verificare influenza della luminosit\'a sui consumi della batteria}
\item{Politica adattativa per limitare il consumo di rete}
\item{Politica adattativa per limitare l'uso della memoria}
\item{Se non si effettuasse il salvataggio in locale quanto si risparmierebbe in termini di energia?}
\item{Fasce energetiche che cambiano dinamicamente - Velocit\'a di scarica della batteria osservata con conseguente adozione della frequenza di pull corrispondente}
\item{Analisi dei consumi differenziato dipendemente dall'operazione effuettuata (Scritture, letture)}
\end{itemize}
...
\makeatother
	...
	La \textit{regressione} formalizza e risolve il problema di una relazione funzionale tra variabili misurate sulla base di dati campionari estratti da un'ipotetica popolazione infinita. Pi\'u formalmente, in statistica la regressione lineare rappresenta un metodo di 		stima del valore atteso condizionato di una variabile dipendente, o endogena, \textit{Y}, dati i valori di altre variabili indipendenti, o esogene, \textit{X}\textsubscript{1},...,\textit{X}\textsubscript{k}: E[\textit{Y}\(\mid\)\textit{X}\textsubscript{1},...,\textit{X}\textsubscript{k}]. Il modello di regressione lineare \'e:
	...
	L'\textit{R}\textsuperscript{2}, o coefficiente di determinazione, \'e una misura della bont\'a dell'adattamento (in inglese \textit{fitting}) della regressione lineare stimata ai dati osservati. \\
...
\bibitem{AWSEC2} \textbf{AWS EC2}: https://aws.amazon.com/it/ec2/?hp=tile\&so-exp=below
\bibitem{AWSDYNAMODB} \textbf{AWS dynamoDB}: https://aws.amazon.com/it/dynamodb/?hp=tile\&so-exp=below
\bibitem{AWSSQS} \textbf{AWS SQS}: https://aws.amazon.com/it/sqs/?hp=tile\&so-exp=below
\bibitem{Coefficiente} \textbf{Regressione Lineare}: https://it.wikipedia.org/wiki/Regressione\_lineare
\bibitem{Regressione} \textbf{Coefficiente di Determinazione}: https://it.wikipedia.org/wiki/Coefficiente\_di\_determinazione
\bibitem{NodeJS} \textbf{Node.js}: https://nodejs.org/it/
\bibitem{ExpressoJS} \textbf{Express.js}: http://expressjs.com/it/
\bibitem{Android} \textbf{Android e Sviluppo}: https://developer.android.com/guide/index.html 
\bibitem{Androidbattery} \textbf{Battery-Historian}: https://developer.android.com/studio/profile/battery-historian.html
\bibitem{sqlite} \textbf{SQLite}: https://www.sqlite.org/index.html
\end{thebibliography}
