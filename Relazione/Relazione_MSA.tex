...
Per effettuare le misurazioni della grandezza in esame, \'e stato utilizzato il \textit{tool Battery-Historian}\footnote{Per maggiori dettagli vedere \textit{Battery-Historian} nella bibliografia} che permette l'analisi e la visualizzazione del consumo energetico generato da tutte le applicazioni eseguite durante il test, definendo i consumi dettati dall'utilizzo del display e della rete da parte da tali applicazioni.
\\\\
...
\\
Inoltre il sistema proposto garantisce tutti i tipi di disaccoppiamento (\textit{spaziale, temporale e di sincronia}) che sono propri di un connettore \textit{Publish-Subscribe}, permettendo all'utente di poter ricevere aggiornamenti continui sui \textit{Topic} di interesse senza inficiare eccessivamente le risorse (limitate) a disposizione del dispositivo. Tale risultato \'e stato possibile soprattutto per merito del sistema a code di messaggi \textit{AWS SQS}, che permette di disaccoppiare completamente i componenti dell'architettura. 
