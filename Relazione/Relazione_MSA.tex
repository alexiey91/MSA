...
Dai primi esperimenti \'e stata notata una possibile relazione funzionale (di tipo lineare) tra due grandezze di interesse: \textbf{frequenza di pull} dei client dalle loro code dedicate e \textbf{velocit\'a di consumo della batteria}. Per avallare questa intuizione sono stati effettuati diversi test che sfruttano la teoria della \textit{regressione lineare}. 
...
L'oggetto dell'analisi \'e di dimostrare l'esistenza di una relazione funzionale tra la frequenza di \textit{pull} (e quindi frequenza di utilizzo della scheda di rete) e la velocit\'a di consumo della batteria, mediante un modello di \textit{regressione lineare}. Bisogna pertanto mostrare che la presunta relazione che sussiste tra le due grandezze sia di tipo lineare, andando ad analizzare l'indice di bont\'a del \textit{fitting} (ossia dell'adattamento della regressione lineare stimata ai dati osservati).
\\
Quando si parla di velocit\'a di consumo della batteria si intende la derivata rispetto al tempo del livello di batteria, ovvero: 
